...
@section('content')
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="row">
            ...
            <div class="col-md-4">
                <a href="{{ route('admin.blogs.create') }}" class="btn btn-primary" style="float: right">Add Blog</a>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <table class="table table-striped">
                    ...
                    <tbody>
                        @foreach ($blogs as $item)
                            <tr>
                                <td><img src="{{ asset($item->image) }}" class="blogImage"  alt=""></td>
                                <td>{{ $item->title }}</td>
                                <td>{{ $item->blog_by }}</td>
                                <td>{{ date('M, d Y',strtotime($item->date)) }}</td>
                                <td>
                                    <a href="{{ url('admin/blogs/'.$item->id.'/edit') }}" ><i class="fa-solid fa-pen-to-square"></i></a>
